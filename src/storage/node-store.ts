...
import { encodeU128, encodeU136, encodeU64 } from '../consensus/primitives';
...
export class NodeStore {
  ...
  public persistUndo(block: ConnectedBlock): void {
  ...
  public persistDerivedState(snapshot: ChainStateSnapshot, activeChainIds: readonly string[]): void {
  ...
  public persistMempool(entries: readonly { txId: string; actualFee: bigint; minimumBurn: bigint; priorityFee: bigint; receivedSeq: bigint; inputs: readonly { sourceId: Buffer; outputIndex: number }[] }[]): void {
    ...
    const transaction = this.connection.transaction(() => {
      ...
      for (const entry of entries) {
        ...
        insertMempool.run(txId, encodeU136(entry.actualFee), encodeU136(entry.minimumBurn), encodeU136(entry.priorityFee), Number(entry.receivedSeq));
